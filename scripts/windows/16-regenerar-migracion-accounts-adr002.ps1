...
$ScriptVersion = "ADR002-MIGRATIONS-V4-20260725"
...
function Invoke-Docker {
    ...
    try {
        # Windows PowerShell 5.1 puede convertir mensajes normales escritos
        # por Docker en stderr en NativeCommandError. Se usa Continue y se
        # valida exclusivamente el código de salida del proceso.
...
$dockerInfoExitCode = Invoke-Docker `
    -Description "validación de Docker Desktop" `
...
Invoke-Docker `
    -Description "detener monolith" `
    -DockerArguments @("compose", "stop", "monolith") `
    -IgnoreFailure |
Out-Null
...
if ($BuildAndStart) {
    ...
    Invoke-Docker `
        -Description "consultar estado de monolith" `
        -DockerArguments @("compose", "ps", "monolith") |
    Out-Null

    Invoke-Docker `
        -Description "consultar logs de monolith" `
        -DockerArguments @(
            "compose",
            "logs",
            "--since=2m",
