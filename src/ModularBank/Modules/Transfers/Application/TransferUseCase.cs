    ...
    ILogger<TransferUseCase> logger)
{
    ...
    public async Task<Transfer> ExecuteAsync(
        Guid userId,
        TransferRequest request)
    {
        var owned = await accountsService.FindByOwnerAsync(userId);
        if (!owned.Any(account => account.Id == request.SourceAccountId))
        {
        ...
        await accountsService.DebitAsync(
            request.SourceAccountId,
            amount,
            request.Reference);

        await accountsService.CreditAsync(
        ...
        // Notifications es una capacidad secundaria. Una falla temporal del
        // microservicio no revierte ni invalida la transferencia ya confirmada.
        // La deuda aceptada es que, hasta implementar Outbox + RabbitMQ, una
        // notificación fallida solo quedará registrada en logs.
        try
        {
            await notificationsService.SendAsync(
                ...
                idempotencyKey: $"transfer-sent:{transfer.Id:N}");
        }
        catch (Exception exception)
        {
            logger.LogWarning(
                exception,
                "La transferencia {TransferId} fue confirmada, pero no se pudo crear su notificación.",
                transfer.Id);
        }
    ...
    public async Task<List<Transfer>> GetHistoryAsync(
        Guid userId,
        Guid accountId)
    {
        var owned = await accountsService.FindByOwnerAsync(userId);
        if (!owned.Any(account => account.Id == accountId))
        {
            throw new UnauthorizedAccessException(
        ...
        return await db.Transfers
            .Where(transfer =>
                ...
                || transfer.TargetAccountId == accountId)
